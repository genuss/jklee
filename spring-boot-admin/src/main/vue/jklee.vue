  ...
  methods: {
    ...
    async updateResultsList() {
      try {
        ...
        if (formFields) {
          if (formFields.sessionName) this.profileRequest.sessionName = formFields.sessionName;
          if (formFields.rawArguments) this.profileRequest.rawArguments = formFields.rawArguments;
          if (formFields.duration) this.profileRequest.duration = this.formatDuration(formFields.duration);
          if (formFields.format) this.profileRequest.format = formFields.format;
        }
      } catch (error) {
        this.error = error;
      }
